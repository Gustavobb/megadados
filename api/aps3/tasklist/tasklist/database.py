...
class DBSession:
    ...
    ###### users
    def read_users(self):
        query = 'SELECT BIN_TO_UUID(uuid_), name FROM users'
    ...
    def remove_user(self, uuid_):
        ...
        with self.connection.cursor() as cursor:
            cursor.execute(
                'DELETE FROM users WHERE uuid_=UUID_TO_BIN(%s)',
                (str(uuid_), ),
            )
        self.connection.commit()
    ...
    def __user_exists(self, uuid_: uuid.UUID):
        with self.connection.cursor() as cursor:
            cursor.execute(
                '''
                SELECT EXISTS(
                    SELECT 1 FROM users WHERE uuid_=UUID_TO_BIN(%s)
                )
                ''',
                (str(uuid_), ),
            )
